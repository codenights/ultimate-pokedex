...
const DIR = path.join(__dirname, "../dev-server/data/image/sprite");
const DIR_SHINY = path.join(__dirname, "../dev-server/data/image/sprite-shiny");
...
async function getPokemonNameFromNationalId(nationalId) {
  return (
    await readJSON(
      path.join(
        __dirname,
        "../dev-server/data/pokemon-species",
        `${nationalId}.json`
      )
    )
  ).name;
