      ...
      <div className="flex w-full">
        <div className="w-5/12 mt-6">
          <InnerCard>
            ...
            {Object.keys(groupBy(weaknesses, "multiplier")).map(
              damageMultiplier => (
                <div className="mb-2" key={damageMultiplier}>
                  <span className="mb-2 mr-2">
                    ⨉ <span className="text-gray-400">{damageMultiplier}</span>{" "}
                    damage from{" "}
                  </span>
                  {weaknesses
                    .filter(
                      damage => damage.multiplier === Number(damageMultiplier)
                    )
                    .map(damageType => (
          ...
          <InnerCard>
            ...
            {Object.keys(groupBy(resistance, "multiplier")).map(
              damageMultiplier => (
                <div className="mb-2" key={damageMultiplier}>
                  <span className="mb-2 mr-2">
                    ⨉ <span className="text-gray-400">{damageMultiplier}</span>{" "}
